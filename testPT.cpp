//This test implementation is similar to testMH, but with parallel tempering.
#include <valarray>
#include <iostream>
#include <fstream>
#include "ptMCMC.hh"
...
double height=64.0;
int Nprop_set=4;
//int Nptc=6; //Number of parallel temperature chains
//int Tmax=6; //maximum temperature
int Nptc=1; //Number of parallel temperature chains
int Tmax=0; //maximum temperature
...
double for_peak_position(state s){
  ...
  bool eq=true;
  for(int i=0;i<Ndim;i++)eq=eq&&(peak[i]==ref_peak[i]);
  return (eq?1.0:0.0)*s.params[ref_dir];
};
...
int main(int argc, char*argv[]){
    ...
    if(argc!=5||!(istringstream(argv[1])>>seed)){
        ...
        cout<<"Usage testMH seed N-steps proposal_option\n(seed) should bin in [0,1)."<<endl;
        return -1;
    }
  ...
  uniform_dist_product prior(mins,maxs);
  sigmas.resize(Ndim,1.0);
  x0s.resize(Ndim,0.5);
  //gaussian_dist_product prior(x0s,sigmas);
  
  //set the likelihood
  llike=new sines(height,ks,mins,maxs,peakbias);
  ...
  cout<<"Parallel tempering: Nptc="<<Nptc<<", Tmax="<<Tmax<<endl;
  cout<<"Beginning runs with label: "<<outname<<endl;
  ostringstream ss("");
  ss<<"testMH_"<<outname<<".out";
  ofstream out(ss.str().c_str());
  ss.str("");ss<<"testMH_"<<outname<<"_eval.out";
  ofstream oute(ss.str().c_str());
  ss.str("");  ss<<"testMH_"<<outname<<"_PTstats.out";
  ...
  for(int ic=0;ic<Nchain;ic++){
    ...
    //Create the chain set
    //MH_chain c(llike,&prior);
    //MH_chain b(llike,&prior);
    //b=c;
    parallel_tempering_chains ptc(Nptc,Tmax);
    //test_prop->set_chain(&c);

    //Initialize
    //c.initialize(Ninit);
    ptc.initialize(llike,&prior,Ninit);
    ptc.set_proposal(*test_prop);
    ...
    for(int i=0;i<ncount;i++){
      ...
      ref_peak=ii;
      P_est[ic][i]=ptc.c0().expectation(for_peak_likelihood,nburn);
      est_peak_loc[ic][i].resize(Ndim);
      double varval=0;
      for(int j=0;j<Ndim;j++){
        ref_dir=j;
	double val=ptc.c0().expectation(for_peak_position,nburn);
        net_peak_loc[i][j]+=val/Nchain;
	val/=P_est[ic][i];
	varval+=ptc.c0().variance(for_peak_position,val,nburn);
