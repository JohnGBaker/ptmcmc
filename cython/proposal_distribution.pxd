...
cdef extern from "../proposal_distribution.hh":
    ...
    cdef cppclass user_gaussian_prop(proposal_distribution):
        user_gaussian_prop(void *user_parent_object, bool (*function)(const void *parent_object, void* instance_object, const states.state &, const vector[double] &randoms, vector[double] &covarvec),const states.stateSpace &sp,const vector[double] &covarvec, int nrand, const string label,void * (*new_user_instance_object_function)(void*object,int id)) except +
        user_gaussian_prop(void *user_parent_object, bool (*function)(const void *parent_object, void* instance_object, const states.state &, const vector[double] &randoms, vector[double] &covarvec),const states.stateSpace &sp,const vector[double] &covarvec, int nrand, const string label) except +
